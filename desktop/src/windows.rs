use std::mem;
use winapi::um::shellapi::{NIM_MODIFY, NIM_DELETE, NOTIFYICONDATAW, NIM_ADD, Shell_NotifyIconW, NIF_ICON, NIF_MESSAGE, NIF_TIP};
use winapi::shared::windef::{POINT, HWND, HBRUSH, HICON, HMENU};
use winapi::um::winuser::*;
use winapi::shared::minwindef::{UINT, WPARAM, HINSTANCE, LPARAM, LRESULT};
use winapi::shared::ntdef::LPSTR;
use std::ptr::null_mut;
use std::cell::RefCell;
use winapi::um::wingdi::{WHITE_BRUSH, GetStockObject};
use crate::wallpaper;
...
thread_local!{
    static H_MENU:RefCell<HMENU> = RefCell::new(null_mut());
    static NID:RefCell<NOTIFYICONDATAW> = RefCell::new(unsafe{std::mem::zeroed()});
    static wm_taskbar_created:UINT = unsafe{ RegisterWindowMessageW(str_to_ws("TaskbarCreated").as_ptr()) };
}
    ...
    match u_msg{
        WM_CREATE => {
            NID.with(|nid|{
                ...
                nid.hIcon = LoadImageW(0 as HINSTANCE, str_to_ws("icon.ico").as_ptr(), IMAGE_ICON, 0, 0, LR_LOADFROMFILE) as HICON;//图标
                nid.szTip.get_mut(0..app_name.len()).unwrap().copy_from_slice(&app_name);
            ...
            H_MENU.with(|menu|{ *menu.borrow_mut() = h_menu; });
        }
        WM_USER => {
            match l_param as u32{
                WM_LBUTTONDBLCLK => { SendMessageW(h_wnd, WM_CLOSE, w_param, l_param); }
                WM_RBUTTONDOWN | WM_LBUTTONDOWN => {
                    ...
                    match TrackPopupMenu(H_MENU.with(|menu|{ *menu.borrow_mut()}),TPM_RETURNCMD, pt.x, pt.y, 0, h_wnd, null_mut()) as usize{//显示菜单并获取选项ID
                        IDR_EXIT => {
...
pub fn win_main(hInstance: HINSTANCE, _hPrevInstance:HINSTANCE, _szCmdLine:LPSTR, iCmdShow: i32) -> i32{
    let app_name = str_to_ws(APP_NAME);

    let handle = unsafe{ FindWindowW(null_mut(), app_name.as_ptr()) }; 
    if !handle.is_null(){
        unsafe{ MessageBoxW(null_mut(), str_to_ws("程序已经运行").as_ptr(), app_name.as_ptr(), MB_OK); }
        return 0;
    }
    ...
    if unsafe{ RegisterClassW(&wndclass) == 0 }{
        unsafe{ MessageBoxW(null_mut(), str_to_ws("程序需要在Windows NT运行！").as_ptr(), app_name.as_ptr(), MB_ICONERROR); }
        return 0;
    }

    // 此处使用WS_EX_TOOLWINDOW 属性来隐藏显示在任务栏上的窗口程序按钮
    let hwnd = unsafe{CreateWindowExW(WS_EX_TOOLWINDOW,
        app_name.as_ptr(), app_name.as_ptr(),
        WS_POPUP,
        CW_USEDEFAULT,
        CW_USEDEFAULT,
        CW_USEDEFAULT,
    ...
    unsafe{
        ...
        while GetMessageW(&mut msg, null_mut(), 0, 0) !=0{
            match msg.message{
                MSG_ERROR => {  MessageBoxW(hwnd, str_to_ws("图片下载出错!").as_ptr(), app_name.as_ptr(), MB_OK); }
                MSG_OK => {
                    NID.with(|nid|{
                        let mut nid = nid.borrow_mut();
                        ...
                        let tip = str_to_ws("壁纸下载完成");
                        nid.szTip.get_mut(0..tip.len()).unwrap().copy_from_slice(&tip);
